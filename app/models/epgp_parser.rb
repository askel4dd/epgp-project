class EpgpParser 
  ...
  def guild_owner(current_user)
    @source_data ||=JSON.parse(source)
    g = Guild.find_by(name: @source_data['guild'], region: @source_data['region'], server: @source_data['realm'])
    return true if g.nil?
    return g.user == current_user ? true : false
  end

  def save(current_user)
    @guild = current_user.guilds.find_or_create_by(name: @source_data['guild'], region: @source_data['region'], server: @source_data['realm'])
